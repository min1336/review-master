...
class CarModelRepository:
    ...
    async def _get_branch_car_model_ids(
        ...
        """
        result = await (
            self._client.table("branch_car_models")
            .select("car_model_id, car_models_master(id, model_name)")
        ...
        for row in result.data:
            master = row.get("car_models_master") or {}
            mid = master.get("id")
            name = master.get("model_name", "")
            if mid and name:
    ...
    async def get_car_model_tags(
        ...
        # monthly_car_model_tag_stats에서 태그 통계 조회
        all_rows: list[dict] = []
        batch_size = 100
        for i in range(0, len(car_model_ids), batch_size):
            batch_ids = car_model_ids[i : i + batch_size]
            result = await (
                self._client.table("monthly_car_model_tag_stats")
                .select("car_model_id, tag_id, positive_count, negative_count, neutral_count")
                .in_("car_model_id", batch_ids)
                .execute()
            )
        ...
        for row in all_rows:
            ...
            agg[key]["positive"] += row.get("positive_count", 0) or 0
            agg[key]["negative"] += row.get("negative_count", 0) or 0
            agg[key]["neutral"] += row.get("neutral_count", 0) or 0
    ...
    async def get_vehicle_tags_raw(self, branch_id: int) -> dict:
        ...
        for (cm_id, tag_id), counts in agg.items():
            car_model = id_to_name.get(cm_id, "기타")
            ...
            car_data[car_model]["total_positive"] += counts["positive"]
            car_data[car_model]["total_negative"] += counts["negative"]
    ...
    async def _get_tag_info_with_category(
        ...
        for row in result.data:
            ...
            mapping[row["id"]] = (row["name"], cat.get("name", ""))
        return mapping
