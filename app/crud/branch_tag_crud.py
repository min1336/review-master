...
class BranchTagRepository(BaseRepository[BranchTag]):
    ...
    async def get_by_branch(
        ...
        ).eq('branch_id', branch_id).eq('period_type', period_type) \
    ...
    async def get_batch_top_tags(
        ...
        # 태그 이름 조회
        tags_result = await self._client.table('tags').select('id, name').execute()
        tag_names = {t['id']: t['name'] for t in tags_result.data}

        # 지점별 태그 조회
        result = await self._client.table(self.table_name).select(
            'branch_id, tag_id, count'
        ).in_('branch_id', branch_ids).eq('period_type', period_type) \
