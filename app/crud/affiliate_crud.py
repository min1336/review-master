...
class AffiliateRepository(BaseRepository[Affiliate]):
    ...
    async def upsert_batch(self, affiliates: List[dict]) -> int:
        ...
        result = await self._client.table(self.table_name) \
            .upsert(rows, on_conflict='affiliate_index').execute()

        return len(result.data) if result.data else 0

    async def get_by_index(self, affiliate_index: int) -> Optional[Affiliate]:
        """업체 인덱스로 조회"""
        result = await self._client.table(self.table_name) \
            .select('*').eq('affiliate_index', affiliate_index).execute()
...
class CarModelRepository(BaseRepository[dict]):
    """car_models 테이블 Repository"""

    model = dict  # 별도 엔티티 없음
    ...
    async def upsert_batch(self, car_models: List[dict]) -> int:
        ...
        result = await self._client.table(self.table_name) \
            .upsert(rows, on_conflict='model_id').execute()

        return len(result.data) if result.data else 0

    async def get_by_model_id(self, model_id: str) -> Optional[dict]:
        """차종 ID로 조회"""
        result = await self._client.table(self.table_name) \
            .select('*').eq('model_id', model_id).execute()
        return result.data[0] if result.data else None

    async def get_all_with_category(self, category: Optional[str] = None) -> List[dict]:
        """카테고리별 차종 목록"""
        query = self._client.table(self.table_name).select('*')
